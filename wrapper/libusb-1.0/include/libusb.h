 ...
 * Date : 05-June-2026
 *
 * libusb.h - clean-room, ABI-compatible subset of libusb-1.0's public header.
 *
 * This ships with the USBIP drop-in libusb replacement. Struct layouts and enum
 * values match upstream libusb-1.0 exactly, member for member and value for value,
 * so a program compiled against the real <libusb.h> can load our libusb-1.0.so.0
 * unchanged. Only the API surface usbip implements is declared here; see the
 * wrapper README for the fidelity matrix.
 *
 * SPDX: the libusb API is LGPL-2.1+; this is an independent reimplementation of
 * its interface for interoperability.
 */
#ifndef LIBUSB_H
#define LIBUSB_H
...
/* On Windows the libusb API is __stdcall (upstream LIBUSB_CALL = WINAPI), leaving the
 * variadic entry point __cdecl, since stdcall cannot carry varargs.
 *
 * Getting it wrong still links: the caller pushes, the callee never pops, and a
 * consumer built against the real header reads garbage (dfu-util saw a NULL device
 * list behind a positive count, and crashed). Spelled out rather than taken from
 * <windows.h>, so the header stays freestanding. */
...
#define LIBUSB_FLEXIBLE_ARRAY            /* C99 [] */
#if defined(__GNUC__)
...
/* ---- enums (values identical to upstream) ----------------------------- */
...
/* ---- descriptor structs (layout matches upstream byte-for-byte) -------- */
...
int LIBUSB_CALL libusb_bulk_transfer(libusb_device_handle *dev_handle,
	unsigned char endpoint, unsigned char *data, int length, int *actual_length, unsigned int timeout);
...
unsigned char * LIBUSB_CALL libusb_get_iso_packet_buffer(struct libusb_transfer *transfer, unsigned int packet);
unsigned char * LIBUSB_CALL libusb_get_iso_packet_buffer_simple(struct libusb_transfer *transfer, unsigned int packet);
...
/* ---- hotplug (stubbed; HAS_HOTPLUG reports 0) ------------------------- */
...
/* ---- static inline helpers (verbatim semantics from upstream) --------- */
static inline uint16_t libusb_cpu_to_le16(const uint16_t x)
{
	union { uint8_t b8[2]; uint16_t b16; } _tmp;
